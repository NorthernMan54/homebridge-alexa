...
function alexaEvent(message) {
  ...
  */
  ...
  var publish = function (callback) {
    debug("Sending message", topic, JSON.stringify(message).substring(0, (JSON.stringify(message).length > 70 ? 70 : JSON.stringify(message).length)));
...
function alexaPriorityEvent(message) {
  var topic = "event/" + username + "/1";
  var publish = function (callback) {
    debug("Sending priority message", topic, JSON.stringify(message).substring(0, (JSON.stringify(message).length > 70 ? 70 : JSON.stringify(message).length)));
    connection.client.publish(topic, JSON.stringify(message), {
      retain: false
    }, callback);
  };
