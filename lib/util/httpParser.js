      ...
      if (body && body.length) {
        if (result.headers['Content-Type'] === 'application/hap+json' || result.headers['Content-Type'] === 'application/json') {
          ...
          try {
            ...
            // TODO: Need to handle the use case of 2 EVENT messages in the same message event
            /*
            Content-Type: application/hap+json
            Content-Length: 54

            {"characteristics":[{"aid":4,"iid":10,"value":false}]}EVENT/1.0 200 OK
            Content-Type: application/hap+json
            Content-Length: 50

            {"characteristics":[{"aid":5,"iid":10,"value":1}]}
            */

            // console.log("ERROR: ", err, messageString);
          }
        } else {
          result.body = body;
        }
