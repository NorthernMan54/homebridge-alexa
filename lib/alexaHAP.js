...
inherits(alexaHAP, EventEmitter);

function alexaHAP(options) {
  pin = options.pin;
...
function _getAccessories(host, port, hapname, callback) {
  ...
  // debug("hapEvent", hapEvent);
  //hapEvent.on('hapEvent', function(res, body) {
  //  debug("EVENT ", res, body);
  //  this.emit('hapEvent', res, body);
  //});
}
...
alexaHAP.prototype.registerEvents = function(message) {
