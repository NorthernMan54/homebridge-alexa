...
function alexaPowerLevelController(message, callback) {
  ...
  try {
    ...
    if (message.directive.endpoint.cookie["TurnOn"]) {
      ...
      reportState.forEach(function(element) {
        ...
        switch (element.interface) {
          ...
          case "Alexa.ThermostatController":
            ...
            var mode = JSON.parse(message.directive.endpoint.cookie["thermostatMode"]);
            debug("mess mode %s -> %s", mode, message.directive.endpoint);
            body = body + spacer + mode.aid + "." + mode.iid;
            var set = JSON.parse(message.directive.endpoint.cookie["targetSetpoint"]);
            spacer = ",";
            body = body + spacer + set.aid + "." + set.iid;
            break;
          default:
...
function alexaEvent(events) {
  ...
  events.forEach(function(event) {
    ...
    alexaLocal.alexaEvent(alexaMessages.eventMessage(event, device));
  });
}
