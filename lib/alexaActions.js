...
function registerEvents(message) {
  ...
  for (var key in message) {
    ...
    var x = {
      "homebridgeName": endpoint.homebridgeName
    };
  ...
  for (var register in HBMessage) {
    ...
    debug("Event Register %s ->", hbInstance.homebridgeName, HBMessage[register]);
    homebridge.HAPeventByName(hbInstance.homebridgeName, JSON.stringify(HBMessage[register]), function(err, status) {
      if (!err) {
        // debug("Registered Event %s:%s ->", hbInstance.host, hbInstance.port, status);
      } else {
        debug("Error: Event Register %s:%s ->", hbInstance.homebridgeName, err, status);
      }
    });
  }
}
...
function alexaColorTemperatureController(message, callback) {
  ...
  switch (action.toLowerCase()) {
    ...
    case "increasecolortemperature":
      ...
      homebridge.HAPstatusByName(haAction.homebridgeName, "?id=" + haAction.aid + "." + haAction.iid, function(err, status) {
        this.log("ColorTemperatureController-get", action, haAction.homebridgeName, status, err);
        ...
        homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
          this.log("ColorTemperatureController-change", action, haAction.homebridgeName, status, body, err);
    ...
    case "setcolortemperature":
      ...
      homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
        this.log("ColorTemperatureController-set", action, haAction.homebridgeName, status, body, err);
...
function alexaPlaybackController(message, callback) {
  ...
  homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
    this.log("PlaybackController", action, haAction.homebridgeName, status, err);
...
function alexaInputController(message, callback) {
  ...
  homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
    this.log("InputController", action, haAction.homebridgeName, status, err);
    var response = alexaMessages.alexaResponse(message, status, err);
    callback(err, response);
  }.bind(this));
}
...
function alexaPowerController(message, callback) {
  ...
  homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
    this.log("PowerController", action, haAction.homebridgeName, status, err);
    var response = alexaMessages.alexaResponse(message, status, err);
    callback(err, response);
  }.bind(this));
}
...
function alexaModeController(message, callback) {
  ...
  homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
    this.log("ModeController", action, haAction.homebridgeName, status, err);
    var response = alexaMessages.alexaResponse(message, status, err);
    callback(err, response);
  }.bind(this));
}
...
function alexaLockController(message, callback) {
  ...
  homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
    this.log("alexaLockController", action, haAction.homebridgeName, status, err);
    var response = alexaMessages.alexaResponse(message, status, err);
    callback(err, response);
  }.bind(this));
}
...
function alexaChannelController(message, callback) {
  ...
  homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
    this.log("alexaChannelController", action, haAction.homebridgeName, status, err);
    var response = alexaMessages.alexaResponse(message, status, err);
    callback(err, response);
  }.bind(this));
}
...
function alexaThermostatController(message, callback) {
  ...
  switch (action) {
    case "SetThermostatMode":
      ...
      homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
        this.log("SetThermostatMode", mode, haAction.homebridgeName, status, err);
        var response = alexaMessages.alexaResponse(message, status, err);
        callback(err, response);
      }.bind(this));
      break;
    case "SetTargetTemperature":
      ...
      homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
        this.log("ThermostatController", action, haAction.homebridgeName, status, err);
        var response = alexaMessages.alexaResponse(message, status, err);
        callback(err, response);
      }.bind(this));
      break;
...
function alexaColorController(message, callback) {
  ...
  homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
    this.log("ColorController", action, haAction.homebridgeName, status, err);
    var response = alexaMessages.alexaResponse(message, status, err);
    callback(err, response);
  }.bind(this));
}
...
function alexaRangeController(message, callback) {
  ...
  switch (action.toLowerCase()) {
    case "adjustrangevalue":
      ...
      homebridge.HAPstatusByName(haAction.homebridgeName, "?id=" + haAction.aid + "." + haAction.iid, function(err, status) {
        this.log("RangeController-get", action, haAction.homebridgeName, status, err);
        ...
        homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
          this.log("RangeController-set", action, haAction.homebridgeName, status, body, err);
    ...
    case "setrangevalue":
      ...
      homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
        this.log("RangeController", action, haAction.homebridgeName, status, body, err);
...
function alexaPowerLevelController(message, callback) {
  ...
  switch (action.toLowerCase()) {
    case "adjustpowerlevel":
      // Need to get current value prior to dimming
      homebridge.HAPstatusByName(haAction.homebridgeName, "?id=" + haAction.aid + "." + haAction.iid, function(err, status) {
        this.log("PowerLevelController-get", action, haAction.homebridgeName, status, err);
        ...
        homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
          this.log("PowerLevelController-set", action, haAction.homebridgeName, status, body, err);
    ...
    case "setpowerlevel":
      ...
      homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
        this.log("PowerLevelController", action, haAction.homebridgeName, status, body, err);
        var response = alexaMessages.alexaResponse(message, status, err, powerLevel);
        callback(err, response);
      }.bind(this));
      break;
...
function alexaSpeaker(message, callback) {
  ...
  switch (action.toLowerCase()) {
    case "adjustvolume":
      // Need to get current value prior to dimming
      homebridge.HAPstatusByName(haAction.homebridgeName, "?id=" + haAction.aid + "." + haAction.iid, function(err, status) {
        this.log("Speaker-get", action, haAction.homebridgeName, status, err);
        ...
        if (!err) {
          ...
          homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
            this.log("Speaker-set", action, haAction.homebridgeName, status, body, err);
    ...
    case "setvolume":
      ...
      homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
        this.log("Speaker", action, haAction.homebridgeName, status, body, err);
        var response = alexaMessages.alexaResponse(message, status, err, volume);
        callback(err, response);
      }.bind(this));
      break;
...
function alexaStepSpeaker(message, callback) {
  ...
  switch (action.toLowerCase()) {
    case "adjustvolume":
      ...
      homebridge.HAPcontrolByName(haAction.homebridgeName, JSON.stringify(body), function(err, status) {
        this.log("StepSpeaker", action, haAction.homebridgeName, status, body, err);
        var response = alexaMessages.alexaResponse(message, status, err, volume);
        callback(err, response);
      }.bind(this));
      break;
  ...
  switch (message.directive.header.name.toLowerCase()) {
    case "reportstate": // aka getStatus
      ...
      reportState.forEach(function(element) {
        homebridgeName = element.homebridgeName;
      ...
      homebridge.HAPstatus(homebridgeName, body, function(err, status) {
        debug("reportState", action, homebridgeName, status, err);
...
function alexaEvent(events) {
  ...
  events.forEach(function(event) {
    var x = {
      'homebridgeName': event.homebridgeName,
      'aid': event.aid,
      'iid': event.iid
    };
    var device;
