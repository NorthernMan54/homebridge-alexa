...
function hapDiscovery(options) {
  ...
  homebridge.on('Ready', function (discovered) {
    debug("Ready", discovered);
  ...
  homebridge.on('Update', async function (discovered) {
    debug("Update", discovered);
    alexaDiscovery.call(options, null, function () {
      // debug("Events", options);
      if (options.events) {
        registerEvents(messages.checkEventDeviceList.call(options, hbDevices.toEvents()));
...
function alexaDiscovery(message, callback) {
  ...
  homebridge.HAPaccessories(async function (endPoints) {
    var response = await _alexaDiscovery.call(this, message, endPoints);
    callback(null, response);
  }.bind(this));
}

async function _alexaDiscovery(message, endPoints) {
  debug("_alexaDiscovery", endPoints);
  var response;

  hbDevices = new Homebridges(endPoints, this);
  // debug("RESPONSE - hbDevices", JSON.stringify(hbDevices, null, 2));
  response = hbDevices.toAlexa(this, message);

  // debug("RESPONSE - toAlexa", JSON.stringify(response, null, 2));
  ...
  // alexaLocal.alexaEvent(response);
  return response
}
