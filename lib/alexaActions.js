...
function alexaInputController(message, callback) {
  ...
  homebridge.HAPcontrol(haAction.host, haAction.port, JSON.stringify(body), function(err, status) {
    this.log("InputController", action, haAction.host, haAction.port, status, err);
    var response = alexaMessages.alexaResponse(message, status, err);
    callback(err, response);
  }.bind(this));
}
...
function alexaModeController(message, callback) {
  ...
  homebridge.HAPcontrol(haAction.host, haAction.port, JSON.stringify(body), function(err, status) {
    this.log("ModeController", action, haAction.host, haAction.port, status, err);
    var response = alexaMessages.alexaResponse(message, status, err);
    callback(err, response);
  }.bind(this));
}
...
function alexaRangeController(message, callback) {
  ...
  switch (action.toLowerCase()) {
    case "adjustrangevalue":
      ...
      homebridge.HAPstatus(haAction.host, haAction.port, "?id=" + haAction.aid + "." + haAction.iid, function(err, status) {
        this.log("RangeController-get", action, haAction.host, haAction.port, status, err);
        ...
        homebridge.HAPcontrol(haAction.host, haAction.port, JSON.stringify(body), function(err, status) {
          this.log("RangeController-set", action, haAction.host, haAction.port, status, body, err);
    ...
    case "setrangevalue":
      ...
      homebridge.HAPcontrol(haAction.host, haAction.port, JSON.stringify(body), function(err, status) {
        this.log("RangeController", action, haAction.host, haAction.port, status, body, err);
