...
module.exports = {
  ...
  _alexaDiscoveryDeleteReport: _alexaDiscoveryDeleteReport
};
...
function hapDiscovery(options) {
  ...
  homebridge.on('Ready', function () {
    deviceDiscovery.call(options, null, function () {
...
function alexaDiscovery(message, callback) {
  deviceDiscovery.call(this, message, async (err, response) => {
    if (this.deleteDevices) {
      const currentEndpoints = stripEndpoints(response);
      if (fs.existsSync(this.persist + '/homebridge-alexa-endpointCache.json')) {
        const lastEndpoints = JSON.parse(fs.readFileSync(this.persist + '/homebridge-alexa-endpointCache.json'));
        const deleteReport = await _alexaDiscoveryDeleteReport.call(this, lastEndpoints, currentEndpoints);
        if (deleteReport) {
          debug("Sending device delete message for %s devices", deleteReport);
          alexaLocal.alexaEvent(deleteReport);
          fs.writeFileSync(this.persist + '/homebridge-alexa-endpointCache.json', JSON.stringify(currentEndpoints, null, 2));
        }
      } else {
        fs.writeFileSync(this.persist + '/homebridge-alexa-endpointCache.json', JSON.stringify(currentEndpoints, null, 2));
      }
...
async function _alexaDiscoveryDeleteReport(lastAlexaDiscovery, currentEndpoints) {
  // console.log('alexaDiscoveryDeleteReport', JSON.stringify(lastAlexaDiscovery.event.payload.endpoints));
  var lastEndpoints = (lastAlexaDiscovery ? lastAlexaDiscovery : []);

  // console.log('alexaDiscoveryDeleteReport - lastEndpoints', lastEndpoints.length);
  // console.log('alexaDiscoveryDeleteReport - currentEndpoints', currentEndpoints.length);
  var deleteEndpoints = [];
  if (Array.isArray(lastEndpoints)) {
    lastEndpoints.forEach(function (last, index, array) {
      if (currentEndpoints.findIndex(current => current.endpointId === last.endpointId) === -1) {
        deleteEndpoints.push(last);
      }
    });
  } else {
    // Empty Array
    deleteEndpoints = currentEndpoints;
  }

  // console.log('alexaDiscoveryDeleteReport - deleteEndpoints', deleteEndpoints.length, deleteEndpoints);
  // console.log('alexaDiscoveryDeleteReport - currentEndpoints', currentEndpoints);
  if (deleteEndpoints.length) {
    var DeleteReport = {
      event: {
        header: {
...
function stripEndpoints(message) {
  return (message && message.event ? message.event.payload.endpoints.map(element => { return { endpointId: element["endpointId"] } }) : []);
}

function deviceDiscovery(message, callback) {
  ...
  homebridge.HAPaccessories(function (endPoints) {
    ...
    if (this.debug) {
      fs.writeFileSync(this.persist + '/homebridge-alexa-alexaDiscovery.json', JSON.stringify(response, null, 2));
    }
