...
function endPoints(message, accessories, filter) {
  var devices = [];
  accessories.forEach(function(element) {
    devices.push(_parseHAPtoAlexa(element.host, element.port, element.HBname, element.accessories, filter));
  });
  var response = {
    "event": {
      "header": {
...
function _parseHAPtoAlexa(host, port, hapname, hap, filter) {
  var alexaDevices = {};
  ...
  for (var accessory in hap.accessories) {
    ...
    let iid, name, description, model, manufacturer;
    ...
    for (var service in device.services) {
      ...
      if (serviceType.startsWith("00000043") || serviceType.startsWith("00000047") ||
        ...
        for (var id in device.services[service].characteristics) {
          ...
          debug("device", aid, iid, type.substring(0, 8))
          switch (type.substring(0, 8)) {
            ...
            case "00000011": // Current Temperature
              ...
              ReportState.push(_reportState("Alexa.TemperatureSensor", host, port, aid, iid));
            ...
            case "00000032": // TargetDoorState
              ...
              ReportState.push(_reportState("Alexa.PowerController", host, port, aid, iid));
              cookie["TurnOn"] = _cookieV(host, port, aid, iid,0); // Open
              cookie["TurnOff"] = _cookieV(host, port, aid, iid,1); // Closed
            ...
            case "0000007C": // WindowCoverings
              ...
              ReportState.push(_reportState("Alexa.PowerController", host, port, aid, iid));
              ReportState.push(_reportState("Alexa.PowerLevelController", host, port, aid, iid));
              cookie["TurnOn"] = _cookieV(host, port, aid, iid,100);
              cookie["TurnOff"] = _cookieV(host, port, aid, iid,0);
              cookie["AdjustPowerLevel"] = _cookie(host, port, aid, iid);
              cookie["SetPowerLevel"] = _cookie(host, port, aid, iid);
              capabilities.push(_lookupCapabilities("PowerController"));
              capabilities.push(_lookupCapabilities("PowerLevelController"));
              break;


            case "000000B0": // Active on a Fan 2 aka Dyson
              if (serviceType.startsWith("000000B7")) {
                alexaCommand.push({
                  "command": "Alexa, turn on " + name
                });
                ...
                ReportState.push(_reportState("Alexa.PowerController", host, port, aid, iid));
                cookie["TurnOn"] = _cookieV(host, port, aid, iid,1);
                cookie["TurnOff"] = _cookieV(host, port, aid, iid,0);
                capabilities.push(_lookupCapabilities("PowerController"));
              }
              break;
            ...
            case "000000CE": // Color Temperature
              ...
              ReportState.push(_reportState("Alexa.ColorTemperatureController", host, port, aid, iid));
              cookie["IncreaseColorTemperature"] = _cookie(host, port, aid, iid);
              cookie["DecreaseColorTemperature"] = _cookie(host, port, aid, iid);
              cookie["SetColorTemperature"] = _cookie(host, port, aid, iid);
...
function _reportState(_interface, host, port, aid, iid) {
  return {
...
function _cookie(_interface, host, port, aid, iid) {
  var response = { "host": host,
  "port": port,
  "aid": aid,
  "iid": iid }
...
function _cookieV(_interface, host, port, aid, iid,value) {
  var response = { "host": host,
  "port": port,
  "aid": aid,
  "iid": iid,
...
function _lookupCapabilities(capability) {
  switch (capability) {
    ...
    default:
      ...
      console.log.error("ERROR: alexaTranslator - Missing capability");
      var response = {
        "type": "AlexaInterface",
        "interface": "Alexa.TemperatureSensor",
        "version": "3",
