    ...
    for (var service in device.services) {
      ...
      if (serviceType.startsWith("00000043") || serviceType.startsWith("00000047") ||
        ...
        for (var id in device.services[service].characteristics) {
          ...
          switch (type.substring(0, 8)) {
            ...
            case "00000022": // Motion sensor state
              ...
              capabilities.push(_lookupCapabilities("MotionSensor"));
              if (events && options.filter && options.filter === host + ":" + port) {
                ...
                hapEvents[JSON.stringify(x)] = {
                  "endpointID": endpointID(host, port, aid, iid),
                  "true": "DETECTED",
                  "false": "NOT_DETECTED",
            ...
            case "0000006A": // Contact Sensor State
              ...
              capabilities.push(_lookupCapabilities("ContactSensor"));
              if (events && options.filter && options.filter === host + ":" + port) {
                var x = {
                  'host': host,
                  'port': port,
            ...
            case "00000011": // Current Temperature
              ...
              if (events && options.filter && options.filter === host + ":" + port) {
                hapEvents[host + ":" + port + ",aid: " + aid.toString() + ",iid: " + iid.toString()] = endpointID(host, port, aid, iid);
              }
              break;
        ...
        if (capabilities.length > 1) {
          ...
          deviceEndPointID = endpointID(host, port, aid, iid);
...
function eventMessage(event) {
  ...
  var device = hapEvents[JSON.stringify(x)];
  var message;
  if (device) {
