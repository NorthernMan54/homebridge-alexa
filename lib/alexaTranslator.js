...
function endPoints(message, accessories, options) {
  ...
  accessories.forEach(function(element) {
    devices.push(_parseHAPtoAlexa(element.host, element.port, element.HBname, element.accessories, options));
  });
...
function _parseHAPtoAlexa(host, port, hapname, hap, options) {
  var alexaDevices = {};
  let acookie = {};
  let ycookie = {};
  ...
  for (var accessory in hap.accessories) {
    ...
    for (var service in device.services) {
      ...
      if (serviceType.startsWith("00000043") || serviceType.startsWith("00000047") ||
        ...
        serviceType.startsWith("00000085")) {
        ...
        for (var id in device.services[service].characteristics) {
          ...
          description = messageUtil.fixHapName(hapname) + " " + name + " " + messageUtil.lookupFriendlyDisplayCategory(serviceType);
          ...
          switch (type.substring(0, 8)) {
            ...
            case "00000022": // Motion sensor state
              ...
              if (events && (options.filter ? options.filter === host + ":" + port : true)) {
                var x = {
                  'host': host,
                  'port': port,
                  'aid': aid,
                ...
                hapEvents[JSON.stringify(x)] = {
                  "endpointID": messageUtil.endpointID(host, port, aid, iid),
                  "true": "DETECTED",
                  "false": "NOT_DETECTED",
            ...
            case "00000011": // Current Temperature
              ...
              capabilities.push(messageUtil.lookupCapabilities("TemperatureSensor", events));
              if (events && (options.filter ? options.filter === host + ":" + port : true)) {
                var x = {
                  'host': host,
                  'port': port,
            ...
            case "00000008": // Brightness
              ...
              if (isSpeaker()) { // Alexa.Speaker and Alexa.StepSpeaker
                ...
                displayCategories = ["SPEAKER"];
                description = messageUtil.fixHapName(hapname) + " " + name + " Speaker";
        ...
        if (capabilities.length > 1) {
          ...
          deviceEndPointID = messageUtil.endpointID(host, port, aid, iid);

          if ((options.filter && options.filter === host + ":" + port) || !options.filter) {
            alexaDevices[host + ":" +
