...
describe("Local Test", () => {
  ...
  describe.only("Discover Accessories", () => {
    const hapEventMock = jest.fn((data) => { }); // console.log('called', data)
    ...
    test("Initial Discovery", async () => {
      await sleep(4500);
      expect(logSpy).toHaveBeenCalledTimes(1);
      expect(logSpy).toHaveBeenCalledWith('alexaDiscovery - returned %s devices', 1);
    ...
    test("add second accessory and increment c#", async () => {
      ...
      await sleep(4500);
      expect(logSpy).toHaveBeenCalledTimes(1);
      expect(logSpy).toHaveBeenCalledWith('alexaDiscovery - returned %s devices', 2);
    }, 5000);
