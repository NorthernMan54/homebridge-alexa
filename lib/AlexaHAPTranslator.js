...
module.exports = {
  ...
  alexaStateErrorResponse: alexaStateErrorResponse
};
...
function alexaResponseSuccess(message,status,err) {
  var now = new Date();
...
function alexaStateResponse(message, reportState, status) {
  ...
  switch (message.directive.header.name.toLowerCase()) {
    case "reportstate":
      var response = {
        "context": {
          "properties": _StateToProperties(reportState, status),
        },
        "event": {
          "header": {
            "messageId": messageId,
...
function alexaStateErrorResponse(message, err) {
  var endpointId = message.directive.endpoint.endpointId;
  var messageId = message.directive.header.messageId;
  var correlationToken = message.directive.header.correlationToken;
  ...
  debug("alexaStatusErrorResponse", JSON.stringify(response));
  return response;
}
