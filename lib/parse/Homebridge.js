...
Homebridge.prototype.toAlexa = function(opt) {
  ...
  // Alexa devices made up of multiple homekit accessories ie AppleTV
    ...
    for (var atv in this.atv) {
      ...
      for (var index in this.accessories) {
        ...
        if (accessory.name === "Play (" + atv + ")" || accessory.name === "Pause (" + atv + ")" || accessory.name === "Menu (" + atv + ")") {
          cookie[messages.atvNameTranslate(accessory.name)] = accessory.toCookie("On", {
            host: this.host,
            port: this.port,
            homebridge: this.homebridge,
            id: this.id,
      ...
      capabilities = capabilities.concat(messages.lookupCapabilities("ATVPlaybackController"));
      list = list.concat({
