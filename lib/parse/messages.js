  ...
  switch (capability) {
    ...
    case "ThermostatController":
      ...
      ops.forEach(function(key) {
        if (key === "thermostatMode" || key === "upperSetpoint" || key === "lowerSetpoint" || key === "targetSetpoint") {
          supported.push({
            "name": key
          });
        }
...
function isYamaha(device) {
  // debug("DEVICE", device);
  if (device.info.Manufacturer === "yamaha-home" && device.info.Name.substr(0, device.info.Name.indexOf(" ")) === "Spotify" && yamahaButton(device.name)) {
    return true;
  } else {
    return false;
  }
