...
function stateToProperties(reportState, hbResponse, message) {
  var properties = [];
  var now = new Date();
    ...
    switch (element.interface.toLowerCase()) {
      case "alexa.speaker":
        properties.push({
          ...
          "name": "volume",
          "value": round(_getValue(element, hbResponse).value),
      ...
      case "alexa.modecontroller":
        properties.push({
          ...
          "name": "mode",
          "value": _getValue(element, hbResponse).value ? "Position.Down" : "Position.Up",
          "timeOfSample": now.toISOString(),
          "uncertaintyInMilliseconds": 500
        });
        break;
      case "alexa.colortemperaturecontroller":
        properties.push({
          ...
          "value": round(1000000 / _getValue(element, hbResponse).value),
          "timeOfSample": now.toISOString(),
          "uncertaintyInMilliseconds": 500
        });
        break;
      ...
      case "alexa.temperaturesensor":
        properties.push({
          ...
          "name": "temperature",
          "value": {
            "value": round(_getValue(element, hbResponse).value, 1),
            "scale": "CELSIUS"
          },
          "timeOfSample": now.toISOString(),
          "uncertaintyInMilliseconds": 500
      ...
      case "alexa.thermostatcontrollerthermostatmode":
        properties.push({
          "namespace": "Alexa.ThermostatController",
          "name": "thermostatMode",
          "value": _getThermoMode(_getValue(element, hbResponse).value),
          "timeOfSample": now.toISOString(),
          "uncertaintyInMilliseconds": 500
        });
        break;
...
function lookupCapabilities(capability, options, operations) {
  var response = [];
...
function _getChannelCookie(device, accessories) {
  ...
  for (var endpoint in accessories.event.payload.endpoints) {
    if (device.name === accessories.event.payload.endpoints[endpoint].friendlyName &&
      device.manufacturer === accessories.event.payload.endpoints[endpoint].manufacturerName) {
      // debug(accessories.event.payload.endpoints[endpoint].cookie);
      if (accessories.event.payload.endpoints[endpoint].cookie.SetPowerLevel) {
        cookie[device.alexaName] = accessories.event.payload.endpoints[endpoint].cookie.SetPowerLevel;
        return (cookie);
      }
    }
  }
...
function _getValue(element, hbResponse) {
  ...
  if (element.interface.toLowerCase() === "alexa.colorcontroller") {
    value = {
      "hue": round(_getHBValue(element.hue, hbResponse).value, 1),
      "saturation": round(_getHBValue(element.saturation, hbResponse).value / 100, 4),
      "brightness": round(_getHBValue(element.brightness, hbResponse).value / 100, 4)
    };
