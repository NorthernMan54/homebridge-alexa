...
function Service(devices, context) {
  ...
  var endpointReg = /\/|\+/g;                 // Invalid characters in endpointid
  ...
  devices.characteristics.forEach(function(element) {
    ...
    if (element.type.substring(0, 8) === '00000023' && element.description === "Name") {
      ...
      this.endpointID = Buffer.from(this.id + "-" + context.homebridge + "-" + context.info.Manufacturer + "-" + this.name + "-" + this.serviceType).toString('base64').replace(endpointReg, '#');
