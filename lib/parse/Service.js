...
function Service(devices, context) {
  ...
  this.endpointID = Buffer.from(this.id + "-" + context.homebridge + "-" + context.info.Manufacturer + "-" + context.info.Name + "-" + this.serviceType).toString('base64').replace('/', '');
  ...
  devices.characteristics.forEach(function(element) {
    ...
    if (element.type.substring(0, 8) === '00000023' && element.description === "Name") {
      ...
      if (context.info.Manufacturer === "Nest") {
        ...
        // debug("Issue #242 %s - %s", element.value, context.info.Name);
      }

      this.endpointID = Buffer.from(this.id + "-" + context.homebridge + "-" + context.info.Manufacturer + "-" + this.name + "-" + this.serviceType).toString('base64').replace('/', '');
